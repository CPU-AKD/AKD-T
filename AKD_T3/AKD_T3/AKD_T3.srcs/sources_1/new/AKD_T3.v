...
module AKD_T3 #(
    ...
    wire[5 + widthd + widthi - 1:0] pipe_WB;
    /*
        [68:64]waddr_WB_reg
        [63:32]result_alu
        [31: 0]inst
    */
    wire wb_reg = |pipe_WB[68:64];
    wire[widthd - 1:0] result1_reg, result2_reg;
    wire[widthd - 1:0] wbdata;
    wire[4:0] raddr1_ID_reg = pipe_ID[14:10] & {5{pipe_ID[31] | pipe_ID[30] & pipe_ID[27] | pipe_ID[29] | pipe_ID[28]}};//如果kj或ldm或alu或st则需要读
    ...
    REG #(widthd, 32) REG(
        .clk    (clk),
        .rst    (rst),
        .ena_low(~(pipe_ID[30] & ~pipe_ID[27] & pipe_ID[26])),
        .raddr1 (raddr1_ID_reg),
        .raddr2 (raddr2_ID_reg),
        .waddr  (wb_reg ? pipe_WB[68:64] : waddr_ID_reg),
        .regdata({
            wb_reg ? wbdata[31:16] : 
            (pipe_ID[30] & ~pipe_ID[27]) ? (//ldi或ldhi?
                pipe_ID[26] ? pipe_ID[20:5] : {16{pipe_ID[20]}}//ldhi?imm:符号扩展
            ) : 16'b0,//选择高16位数据
            wb_reg ? wbdata[15:0] : (
                pc            & {16{jump}} |
                pipe_ID[20:5] & {16{pipe_ID[30] & ~pipe_ID[27] & ~pipe_ID[26]}}//ldi
            )//选择低16位数据
        }),
    ...
    wire[5 + widthd * 2 + widthi - 1:0] pipe_EX;
    /*
        [100:96]waddr_EX_wbreg
        [ 95:64]result1_reg
        [ 63:32]result2_reg
        [ 31: 0]inst
    */
    ...
    Pipeline #(5 + widthd * 2 + widthi) ID_EX(
        .clk(clk),
        .rst(rst),
        .wea(~stall_ID_EX),
        .pipe_prev((hazard_data | hazard_structure | bubble) ? 0 : {waddr_ID_wbreg, result1_reg, result2_reg, pipe_ID}),
        .pipe(pipe_EX)
    );
    ...
    wire[5 + widthd * 3 + widthi - 1:0] pipe_MEM;
    /*
        [132:128]waddr_WB_wbreg
        [127: 96]result1_reg
        [ 95: 64]result2_reg
        [ 63: 32]result_alu
        [ 31:  0]inst
    */
    ...
    Pipeline #(5 + widthd * 3 + widthi) EX_MEM(
        .clk(clk),
        .rst(rst),
        .wea(~stall_EX_MEM),
        .pipe_prev({pipe_EX[100:32], result_alu, pipe_EX[31:0]}),
        .pipe(pipe_MEM)
    );
    ...
    Pipeline #(5 + widthd + widthi) MEM_WB(
        .clk(clk),
        .rst(rst),
        .wea(~stall_MEM_WB),
        .pipe_prev({pipe_MEM[132:128], pipe_MEM[63:0]}),
        .pipe(pipe_WB)
    );
    assign cpu_halt = (~|pipe_WB[31:28]) & pipe_WB[27];
    assign wbdata = (result_mem & {widthd{pipe_WB[30] & pipe_WB[27]}}) | (pipe_WB[63:32] & {widthd{pipe_WB[29]}});//如果ldm就用result_mem作为写回数据，如果alu就用传到WB阶段的alu结果
    Hazard_Checker Hazard_Checker(
        .inst_EX(pipe_EX[31:0]),
        .inst_MEM(pipe_MEM[31:0]),
        .inst_WB(pipe_WB[31:0]),
        .waddr_EX_wbreg(pipe_EX[100:96]),
        .waddr_MEM_wbreg(pipe_MEM[132:128]),
        .waddr_WB_wbreg(pipe_WB[68:64]),
